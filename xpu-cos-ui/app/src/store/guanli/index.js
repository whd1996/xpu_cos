import {reqAlluser,reqAllcommidity,reqAllorder,reqAllpur,reqAdduser,reqDeuser,reqUpdateuser,requp,reqaddsp,reqdelesp,reqoutcx} from '@/api/index';
...
const actions = {
        ...
        async outguali({commit}){
            let result=await reqoutcx();
            console.log(result);
            if(result.flag=true){
                commit('OUTGUANLI','ok')
            }else{
                commit('OUTGUANLI','err')
            }
        }
}
