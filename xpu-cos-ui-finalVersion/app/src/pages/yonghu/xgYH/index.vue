  ...
  <div>
    ...
    v-show="isshow"
     ...
     <el-table-column fixed="right" label="操作" width="100">
        <template >
          <el-row>
            <el-button type="warning" round size="mini" @click="xg"
              >修改</el-button
            >
          </el-row>
        </template>
...
export default {
  name:'XgYonghu',
   ...
   /* console.log(this.$store.state.login.user.username)*/
    await  this.$store.dispatch('getumessgae',this.$store.state.login.user.username);
            // console.log(this.$store.state.yonghu.yonghuu);
       this.tableData=[{
            id:this.$store.state.yonghu.yonghuu.id,
            address:this.$store.state.yonghu.yonghuu.address,
            nickname:this.$store.state.yonghu.yonghuu.nickname,
            roleId:'用户',
            userName:this.$store.state.yonghu.yonghuu.userName,
            userPassward:this.$store.state.yonghu.yonghuu.userPassward,
            userPhone:this.$store.state.yonghu.yonghuu.userPhone,
          },]
    },
    methods: {
      xg(){
